...
| 数据源                  | 抓取方式                    | 关键词来源                               |
| -------------------- | ----------------------- | ----------------------------------- |
| **arXiv**            | 按标签搜索，每 2 关键词合并查询，每关键词最多 3 页，每标签最多 20 条查询，提前结束 | 10 个研究方向标签，每标签 10～20 篇（days=14，max_per_tag=20）。支持 `tag` 参数：选定标签时仅抓取该标签关键词。**3dgs 子标签**：3DGS物理仿真、VR/AR、3DGS水下建模、空间智能 搜索时自动附加 3dgs AND 约束 |
