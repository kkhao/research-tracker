...
| 3DGS物理仿真 | physics simulation, mpm, material point method | **需同时含 3dgs** |
| 具身智能 | embodied ai, robot, robotics | **需同时含 3dgs** |
| 3D重建/生成/渲染 | 3d reconstruction, 3d generation, 3d rendering | — |
| VR/AR | virtual reality, augmented reality | **需同时含 3dgs** |
| 可重光照/逆渲染 | relighting, relightable, inverse rendering, lighting editing | — |
| 3D人体/角色 | human avatar, character animation, 3d human | — |
| 3DGS编辑 | gaussian splatting edit, gaussian edit, 3dgs edit | — |
| 3DGS水下建模 | underwater, 水下 | **需同时含 3dgs** |
| 空间智能 | spatial reasoning, scene understanding, spatial understanding, spatial perception, spatial intelligence | **需同时含 3dgs**（抓取时用原关键词，组合过窄易返回空） |
...
- **逻辑**：
  ...
  - **3dgs 子标签**：3DGS物理仿真、具身智能、VR/AR、3DGS水下建模、空间智能 需**同时**包含 3dgs 相关词（3dgs、3d gaussian、gaussian splatting 等），否则不打标
...
- **逻辑**：
  ...
  - **3dgs 子标签**：同上，3DGS物理仿真、具身智能、VR/AR、3DGS水下建模、空间智能 需同时含 3dgs 相关词
...
- **逻辑**：
  - 在 title + summary 中匹配 POST_TAG_KEYWORDS
  - **3dgs 子标签**：同上，3DGS物理仿真、具身智能、VR/AR、3DGS水下建模、空间智能 需同时含 3dgs 相关词
  - 根据公司归属方向追加 3D重建/生成/渲染、视频/世界模型、3D设计、具身智能
  - 公司名、微信公众号作为额外标签
...
| 数据源                  | 抓取方式                    | 关键词来源                               |
| -------------------- | ----------------------- | ----------------------------------- |
| **arXiv**            | 按标签搜索，每标签单独请求 + 分类限制 + 日期 | 11 个研究方向标签，每标签 10～20 篇（days=14，max_per_tag=20）。支持 `tag` 参数：选定标签时仅抓取该标签关键词。**3dgs 子标签**：3DGS物理仿真、具身智能、VR/AR、3DGS水下建模、空间智能 搜索时自动附加 3dgs AND 约束 |
...
**标签过滤**：支持 `tag` 参数。选定标签时，仅用 POST_TAG_KEYWORDS 中该标签对应的关键词抓取（选定标签→选定时间→抓取）。3dgs 子标签会附加 "3dgs" 搜索词约束。
...
**标签过滤**：支持 `tag` 参数。选定标签时，仅用 POST_TAG_KEYWORDS 中该标签对应的关键词抓取 HN、YouTube。3dgs 子标签会附加 "3dgs" 搜索词约束。Reddit 按子版块抓取无关键词搜索，按标签时跳过。
